...
class SubscriptionsDaoTest : RoverEngageTestBase() {
    ...
    fun subscriptionsDaoUpsertAndClear() = runBlocking {
        ...
        subs.upsertSubscription(SubscriptionEntity("s1", "N", "D", true, "active", "https://example.com/logo.png"))
        assertThat(subs.getSubscriptionById("s1")?.name, equalTo("N"))
        assertThat(subs.getSubscriptionById("s1")?.logoURL, equalTo("https://example.com/logo.png"))
    ...
    fun subscriptionsGetAllReturnsAllSubscriptions() = runBlocking {
        val subs = database!!.subscriptionsDao()
        val sub1 = SubscriptionEntity("s1", "Sub 1", "Desc 1", true, "active", "https://example.com/s1.png")
        val sub2 = SubscriptionEntity("s2", "Sub 2", "Desc 2", false, "archived", null)
        ...
        assertThat(all.map { it.logoURL }, equalTo(listOf("https://example.com/s1.png", null)))
    }

    @Test
    fun subscriptionsUpsertUpdatesExisting() = runBlocking {
        val subs = database!!.subscriptionsDao()
        val original = SubscriptionEntity("s1", "Original", "Desc", false, "active", null)
        subs.upsertSubscription(original)

        val updated = SubscriptionEntity("s1", "Updated", "New Desc", true, "archived", "https://example.com/updated.png")
        ...
        assertThat(fetched?.logoURL, equalTo("https://example.com/updated.png"))
    }
