...
internal class VotingInteractor(
        ...
        fetchVotingResults(optionIds).observeOn(mainScheduler).first().subscribe({ optionResults ->
            if (optionResults.results.isNotEmpty()) {
                when (val state = currentState) {
                    ...
                    is VotingState.InitialState -> currentState = VotingState.InitialState.transitionToResultsSeeded(optionResults)
                    else -> { /* no-op */ }
                }
            } else {
    ...
    fun castVotes(optionId: String) {
        when (val state = currentState) {
            ...
            is VotingState.InitialState -> currentState = VotingState.InitialState.transitionToPollAnswered(optionId)
            else -> { /* no-op */ }
        }
    }
