...
/**
 * Identifier used to scope `/home-view` requests.
 *
 * Use a `UserID` when available; fall back to a device identifier otherwise.
 */
internal sealed class HomeViewIdentifier {
    /**
     * A user identifier for targeted home view requests.
     */
    data class UserID(val value: String) : HomeViewIdentifier()

    /**
     * A device identifier used when no user ID is available.
     */
    data class DeviceIdentifier(val value: String) : HomeViewIdentifier()
}
