...
import io.rover.sdk.experiences.rich.compose.ui.layout.FeatureFlags.fastFlexibilitySort
import io.rover.sdk.experiences.rich.compose.ui.layout.GREATEST_FINITE
...
internal fun Collection<IntrinsicMeasurable>.filterByMaxPriority(): Collection<IntrinsicMeasurable> {
    val measurablesByPriority = associateBy({ it }) { measurable ->
        val priority = measurable.layerModifierData?.layoutPriority
...
/**
 * Group all the measurables by their Experiences layout priority, with the groups ordered by priority.
 *
...
internal fun Collection<Placeable>.filterByMaxPriority(): Collection<Placeable> {
    val measurablesByPriority = associateBy({ it }) { placeable ->
        val priority = placeable.layerModifierData?.layoutPriority
        // if any lack a priority, then just have byMaxPriority return the entire list.
        priority ?: 0
    }
    ...
    val byFlexibility = associateBy({ it }) { measurable ->
        measurable.annotateIntrinsicsCrash {
            if (fastFlexibilitySort) {
                val range = measurable.experiencesHorizontalFlex()

                range.last - range.first
            } else {
                val lower = measurable.experiencesMeasure(
                    Size(0, maxHeight)
                )

                val upper = measurable.experiencesMeasure(
...
internal fun Collection<Measurable>.sortByVerticalFlexibility(maxWidth: Int): List<Measurable> {
    ...
    val byFlexibility = associateBy({ it }) { measurable ->
        measurable.annotateIntrinsicsCrash {
            if (fastFlexibilitySort) {
                val range = measurable.experiencesVerticalFlex()
