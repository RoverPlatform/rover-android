...
class InterpolatorTests {
    ...
    private val testTimeZone = ZoneId.of("America/Toronto")!!
    ...
    private fun checkWhetherExceptionIsThrown(
        ...
        try {
            ...
            ).interpolateWithExceptions(expression)
        } catch (e: Exception) {
            actualException = e
        }
    ...
    fun `expression with no interpolation returns original string`() {
        val result = Interpolator(
            emptyMap(),
            overrideTimeZone = testTimeZone
        ).interpolate(nonInterpolatedString)
    ...
    @Test
    fun `local time date format test`() {
        val result = Interpolator(
            dataContextOf(
                Keyword.DATA.value to mapOf("datetime" to "2022-02-01T19:46:31")
            ),
    ...
    @Test
    fun `dateFormat when given a UTC time zone should convert to device time zone`() {
        val timestamp = "2023-01-05T20:25:00Z"

        val format = "{{dateFormat data.data.kevinTests.dateTime \"EEEE, MMM d, yyyy h:mma z\"}}"
    ...
    fun `prefix with string returns expected string`() {
        val result = Interpolator(
            emptyMap(),
            overrideTimeZone = testTimeZone
        ).interpolate("{{prefix \"Stand by me!\" 8}}")
    ...
    fun `suffix with string returns expected string`() {
        val result = Interpolator(
            emptyMap(),
            overrideTimeZone = testTimeZone
        ).interpolate("{{suffix \"Boom! Kapow!\" 6}}")
        assert(result == "Kapow!")
    }
    ...
    fun `nested expression with string returns expected string`() {
        val result = Interpolator(
            emptyMap(),
            overrideTimeZone = testTimeZone
        ).interpolate("{{ uppercase (suffix (dropFirst \"mr. jack reacher\" 4) 7) }}")
