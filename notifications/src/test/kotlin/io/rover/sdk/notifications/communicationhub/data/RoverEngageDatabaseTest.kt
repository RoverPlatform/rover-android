...
class RoverEngageDatabaseTest : RoverEngageTestBase() {
    ...
    /**
     * Test forward cursor state management and persistence via sync_states
     */
    @Test
    fun testCursorPersistenceAndRetrieval() = runTest {
        ...
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == null) { "Initial cursor should be null" }

        // Test creating first cursor
        syncStateDao.replaceSyncState(SyncStateEntity("posts", forwardCursor = "cursor-page-1", backwardCursor = null, historyComplete = false))
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == "cursor-page-1") { "Should retrieve the same cursor that was saved" }

        // Test updating existing cursor (replace, not duplicate)
        syncStateDao.replaceSyncState(SyncStateEntity("posts", forwardCursor = "cursor-page-2", backwardCursor = null, historyComplete = false))
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == "cursor-page-2") { "Should retrieve the updated cursor" }

        // Test setting cursor to null (clearing it)
        syncStateDao.replaceSyncState(SyncStateEntity("posts", forwardCursor = null, backwardCursor = null, historyComplete = false))
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == null) { "Cursor should be null after clearing" }

        // Test setting cursor again after clearing
        syncStateDao.replaceSyncState(SyncStateEntity("posts", forwardCursor = "cursor-page-3", backwardCursor = null, historyComplete = false))
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == "cursor-page-3") { "Should be able to set cursor again after clearing" }

        // Test multiple entity types are independent
        syncStateDao.replaceSyncState(SyncStateEntity("articles", forwardCursor = "article-cursor-1", backwardCursor = null, historyComplete = false))
        assert(syncStateDao.getSyncState("articles")?.forwardCursor == "article-cursor-1") { "Should handle multiple entity types" }
        assert(syncStateDao.getSyncState("posts")?.forwardCursor == "cursor-page-3") { "posts cursor should be unaffected by articles" }
    }
    ...
    fun testDatabaseTransactionIntegrity() = runTest {
        ...
        val subscriptionItems = listOf(
            ...
            SubscriptionItem(
                ...
                optIn = true,
                status = "unpublished",
                logoURL = "https://example.com/sub-3.png",
            )
        )
        ...
        assert(allSubscriptions.first { it.id == "sub-1" }.logoURL == "https://example.com/sub-1.png") { "Subscription logoURL should persist" }
    ...
    fun testPlaceholderSubscriptionCreation() = runTest {
        ...
        assert(placeholderSub?.logoURL == null) { "Placeholder should have default logoURL = null" }
        ...
        val realSubscriptionItem = SubscriptionItem(
            ...
            optIn = false,
            status = "archived",
            logoURL = "https://example.com/real-logo.png",
        )
        ...
        assert(updatedSubscription?.logoURL == "https://example.com/real-logo.png") { "logoURL should be updated" }
